...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        /* ---- CV Dropdown ---- */
        const cvWrap = document.getElementById('cv-dropdown-wrap');
        const cvBtnEl = document.getElementById('btn-cv');
        const cvDropdownEl = document.getElementById('cv-dropdown');

        const cvFiles = {
            'web-dev': { path: 'assets/cv-webdev.pdf', name: 'BidexTech-WebDev-CV.pdf' },
            'marketing': { path: 'assets/cv-marketing.pdf', name: 'BidexTech-DigitalMarketing-CV.pdf' },
            'combined': { path: 'assets/resume.pdf', name: 'BidexTech-CV.pdf' },
        };

        function downloadCV(key) {
            ...
            fetch(path, { method: 'HEAD' })
                .then(res => {
                    if (res.ok) {
                        ...
                        alert('This CV version will be available soon. Please contact me on WhatsApp.');
                    }
                })
                .catch(() => alert('CV will be available soon. Please contact me on WhatsApp.'));
        }

        if (cvBtnEl && cvWrap) {
            cvBtnEl.addEventListener('click', function (e) {
                e.stopPropagation();
                const isOpen = cvWrap.classList.toggle('open');
                cvBtnEl.setAttribute('aria-expanded', String(isOpen));
            });

            cvDropdownEl?.querySelectorAll('.cv-option').forEach(opt => {
                opt.addEventListener('click', function () {
                    downloadCV(opt.dataset.cv);
                    cvWrap.classList.remove('open');
                    cvBtnEl.setAttribute('aria-expanded', 'false');
                });
            });

            document.addEventListener('click', function (e) {
                if (cvWrap && !cvWrap.contains(e.target)) {
                    cvWrap.classList.remove('open');
                    cvBtnEl.setAttribute('aria-expanded', 'false');
                }
            });
        ...
        // Mobile CV download buttons
        document.querySelectorAll('.nav-mobile-cv-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                downloadCV(btn.dataset.cv);
                closeMenu();
            });
        });
            ...
            form.addEventListener('submit', async function (e) {
                ...
                try {
                    if (typeof emailjs !== 'undefined') {
                         ...
                         */
                        await emailjs.send('service_uqfdhur', 'template_wkbn0ga', {
